...
def compress_point(public_key):
    return hex(public_key.x) + hex(public_key.y % 2)[2:]
end = time.time()
print("The time of execution of key generation is :", end-start)
k=compress_point(public_key)
print("compressed Public key=",k)

cipher2 = onetimepad.encrypt(k, 'random')
print("Cipher key is ")
print(cipher2)
print("Plain text key is ")
msg = onetimepad.decrypt(cipher2, 'random')
...
def test():
    ...
    end2 = time.time()
    print("encryption time=",end2-start2,"\n")
    start3 = time.time()
    decription_bin_message = cipher.decription(encription_bin_message, bin_key)
    end3 = time.time()
